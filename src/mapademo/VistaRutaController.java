...
public class VistaRutaController implements Initializable {
    ...
    public void mostrarActividadEnMapa(Activity activity) {
        ...
        try {
            ...
            if (lblVelocidad != null) lblVelocidad.setText(String.format("%.2f km/h", activity.getAverageSpeed()));
            if (lblRitmo != null) lblRitmo.setText(String.format("%.2f min/km", activity.getAveragePace()));
            ...
            double ancho = img.getWidth() > 0 ? img.getWidth() : 1000.0;
            double alto = img.getHeight() > 0 ? img.getHeight() : 1000.0;

            buildMap(imgFile, ancho, alto);

            MapProjection proj = new MapProjection(region, ancho, alto);
            ...
            for (TrackPoint tp : activity.getTrackPoints()) {
                ...
                if (prevPoint != null && prevP != null) {
                    ...
                    if (!Double.isFinite(p.getX()) || Math.abs(p.getX()) > 10000 ||
                        !Double.isFinite(p.getY()) || Math.abs(p.getY()) > 10000) {
                        continue;
                    }
            ...
            System.out.println("No se encontró el archivo de imagen. Cargando mapa de emergencia.");
            buildMap(new File("maps/upv.jpg"), 1000, 1000);
        }
    }
    ...
    private void zoom(double scaleValue) {
        if (map_scrollpane == null || zoomGroup == null) return;
    ...
    void listClicked(MouseEvent event) {
        if (map_listview == null || mapPane == null || zoomGroup == null || map_scrollpane == null) return;
    ...
    private void buildMap(File imgFile, double ancho, double alto) {
        mapPane = new Pane();
        mapPane.setPrefSize(ancho, alto);
        mapPane.setMinSize(ancho, alto);
        mapPane.setMaxSize(ancho, alto);
        ...
        if (map_scrollpane != null) {
            ...
            map_scrollpane.setPannable(true); // Permite arrastrar el mapa
        }
    }

    private void onMapRightClick(double x, double y) {
        if (mapContextMenu != null && mapPane != null) {
            mapContextMenu.hide();
    ...
    private void addPoi(double x, double y) {
        ...
        if (result.isPresent()) {
            ...
            text.setX(x);
            text.setY(y);
            if (mapPane != null) mapPane.getChildren().add(text);
        }
    }
    ...
    private void cambiarMapa(ActionEvent event) throws IOException {
        ...
        if (zoom_slider != null && zoom_slider.getScene() != null) {
    ...
    private void addCircle(double x, double y) {
        ...
        circle.setCenterX(x);
        circle.setCenterY(y);
        if (mapPane != null) mapPane.getChildren().add(circle);
    }
